...
void plot_icarus_intrinsic(){
	...
	double ICARUS_proposal_intrinsic[11]= {462.502, 824.391, 1018.69, 1029.74, 958.11, 879.789, 819.313, 664.957, 510.596, 412.076, 192.606};
	double ICARUS_proposal_photon[11] = {384.361, 232.405, 107.261, 58.1005, 42.4614, 17.8737, 13.4011, 17.8821, 24.5776, 11.1732, 2.20781};
	double ICARUS_proposal_muon[11] = {64.8011, 64.8011, 42.4514, 35.7575, 33.5162, 24.5776, 15.6424, 8.93519, 6.70223, 11.1665, 20.11};
...
*/	         ti2 =100*(ti-ICARUS_proposal_intrinsic[k]*ebinw[k])/(ICARUS_proposal_intrinsic[k]*ebinw[k]);
	...
	char namei[200];
sprintf(namei, "CC intrinsic #nu_{e} p: %.1f ",100*Nintrin/(607+706.0+180));
	char namep[200];
sprintf(namep, "NC single #gamma p: %.1f ",100*Nphoton/(149.0+9));
	char namem[200];
