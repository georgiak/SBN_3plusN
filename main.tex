...
There have been a variety of methods used in\marcom{PB}{Restructure.} past experiments to bound the
...
produced instead of a light active neutrino. A sufficiently short-lived
sterile, however, could decay back effectively instantly to visible particles
voiding the single lepton cut these analysis implement and thus escaping the
bounds.  These can be powerful probes but are limited in the parameter-space
they can search leaving beam-dump style experiments the only terrestrial
experiments for a large region of sterile masses. Bounds on $U_{\alpha 4}$
from such beam-dump experiments are most often presented in the context of the
minimal model as discussed above. If one then considers enlarged decay rates in
a non-minimal model, care must be taken with existing bounds, as an enlarged
...
\begin{subfigure}[c]{\textwidth}
	\center
	\large
	\resizebox{0.99\columnwidth}{!}{\input{figures/spectrum_angles_ee.tex}}
...
when one soley considers the minimal model. \marcom{PB}{Comment on non-minimal
...
We perform the analysis for energy only (dashed black lines) and for energy and
time-of-flight combined (colored bands). The results\marcom{PB}{Update this?
COmment on three body vs. two body.} of this study can be found in
\reffig{fig:tof_scatter} below in the form of 1 and 2 $\sigma$ ranges of
