...
numbers of sterile events expected, the $\mu$BooNE experiment is significantly
...
%
Non-observation of any excess at $\mu$BooNE would not negatively effect the
program for a subsequent search at SBND or ICARUS as a large amount of their
probable parameter space is not available to $\mu$BooNE.
...
POT at \muboone. Shown\marcom{PB}{This sentence is overkill.} is the predicted
upper limits on sterile mixing for the $\ster \rightarrow \nu_\mu e^+e^-$
(blue, top left), $\ster \rightarrow \pi^+ \mu^-$ (red, top left), $\ster
\rightarrow \nu_e e^+ e^-$ (blue, top right),  $\ster \rightarrow \pi^+ e^-$
(green, top right), $\ster \rightarrow \nu_\mu \gamma$ (purple, middle left),
$\ster \rightarrow \nu_\mu \pi^0$ (yellow, middle left),$\ster \rightarrow
\nu_e \gamma$ (purple, middle right), $\ster \rightarrow \nu_e \pi^0$ (yellow,
middle right), $\ster \rightarrow \nu_e e^\pm \mu^\mp $ (bright green, bottom
...
the background-less 90\% C.L upper limit contours defined as 2.44 events
...
of the cut based background analysis discussed in \refsec{sec:backgroundestimate}
(upper solid colored lines).  
%
...
Additionally,\marcom{PB}{This section needs expanding.} the previously
...
new constraints on exotic sterile signatures, although the potential
backgrounds are larger in these photonic channels leading to a wider spread in
achievable limits. For steriles  $m_\pi^0 \leq m_\ster \leq m_\pi^\pm +m_\mu$
the  $\ster \rightarrow \nu_\mu \pi^0$ channel can extend the limits beyond
that of the traditional $e^+e^-$ searches, even in the minimal model, although
in the case of electron mixing the $\pi^0$ bounds are always below that of the
semi-leptonic $\ster \rightarrow e^\pm \pi^\mp$.  
...
indeed from a heavy particle propagating the baseline before decaying. Timing
is crucial for this, as an energy analysis alone is not capable of providing a
strong enough case that the events arise from a decay-in-flight even if the
energy spectrum is consistent, as we cannot discount mis-understood
...
with beam-related models by generating many Monte-Carlo experiments simulating
...
mass, these results hold for all channels studied, some results from this
analysis for the $\ster \rightarrow e^\pm \pi^\mp$ channel are shown in
\reffig{fig:hockey} . On the left we assume ICARUS observes an excess of 100
events due to a 300 MeV sterile. To guarantee that we obtain $3\sigma$
significant rejection of the beam-window hypothesis 95\% of the time ICARUS
would need a timing resolution of $\leq$ 3.5 ns. The greater the observed
excess the proportionally less stringent timing resolution is required, as can
be seen in the right-hand plot of \reffig{fig:hockey}.
...
be able to distinguish models even if the energy spectra of the decay
...
as $m_\ster$. In this endeavor, timing is a very important tool, being valid in
both the minimal model as described above but also in non-minimal models for
which the total rate depends on additional unconstrained parameters.
Elementary special relativity tells us that for an arrival time delay (behind a
...
lack of missing-energy in the final state.\newtext{PB}{What do you think: cut
the right figure. It's the one where we could have done invariant mass
determination of $m_N$ and it's less impressive...} }
...
$\mu$BooNE is already collecting data and has the potential to observe a signal
