...
contours mark the regions where the detectors in question see more than $2.6$
events, following the procedure of \refref{mmm} designed for backgroundless
...
\caption{\label{fig:no_cuts_scaled_bkg}The sensitivity contours based on the total
...
\item \sout{Add $\pi_0\nu$ decay to plot.} \newtext{PB}{These plots haven't been added here. Did you make them? (I see you've added the channels to the code. Thanks.)}

\item Actually commpute a $95\%$ CL exlusion region for a fair comparison with bounds.

\item \sout{Add $\nu_e$ flux to \emph{flux.c}.}

\item \sout{Work out what has gone into the ``cut-less" plots.}\newtext{PB}{``Cut-less'' plots have weight factors set to 1.} Recompute for reasonable cuts? Vary the backgrounds according the cuts and Mark's estimates?
