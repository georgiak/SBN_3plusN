...
$\ster \rightarrow l^- \pi^+$ whilst, due to the strong particle identification
...
Additionally we show that the if the light detection system employed by SBND
and ICARUS can achieve nano-second timing resolution, it in principle allows
...
distinctive signature for this class of models.
%
}
...
these events are shifted to be above the sterile mass
threshold.\marcom{PB}{This sounds a bit dodgy. How do we chose where to move
them to? First bin? Shared over all bins? Can we clarify?}
...
bounds infered from the non-observation of a given channel, may not hold in 
...
simplified form depending on the size of $\Gamma_\text{T}\lambda/\gamma\beta$:\marcom{PB}{Only the upper one applies to us, right? Is this unnecessary?}
%
\begin{align} 
%
...
$\mathrm{d}\phi/\mathrm{d}E$, is estimated as described in \refsec{seq:prod}
...
probability for any given sterile to subsequently decay scales as
$1/|P_{\ster}|$\marcom{PB}{What did you mean by this, Mark? E?} the fluxes of
...
%
This low-energy bias exagerates the kinematic differences between our
...
about a plot of our weight functions from the cuts?}
%
...
kinematic cuts there is the potential for even greater background supression by
...
is lower, $\approx 10$ns.\marcom{PB}{Can we comment on why?}
...
light counterparts to be distinguishable. Shown in \reffig{fig:timing_line} is
...
sterile shown in \reffig{fig:timing}\marcom{PB}{Where should this point?}. For
the purposes of this study we define the beam-correlated window to be a $4$~ns
period, $1$~ns each side of the $2$~ns beam bucket.
...
extremely reduced beam-correlated backgrounds. \marcom{PB}{We need to comment on cosmogenics.} 
...
anomaly is indeed from the decay of a heavy propagating sterile. \marcom{PB}{We
