...
allowing for a complementary new physics search to eV-scale oscillations. We
provide estimates for the bounds that the full SBN facility can place on the
parameter spaces of these models and demonstrate that the facility can in many
...
\nu_\alpha \pi^0$, which may be enhanced in a non-minimal model. Additionally
we show that the if the light detection system employed by SBN can achieve
nano-second timing resolution, it in principle allows for observable structure
in the temporal distribution of events relative to the beam-buckets. When
combined with information from the energy spectrum, this allows SBN to act in a
