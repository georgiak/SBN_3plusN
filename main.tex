...
oscillations, without the need for any beam or detector modifications. We
...
between $1-493$ MeV. Such a search is entirely compatible with the main goals
of the SBN, being possible with no additional modifications to either the
neutrino beam nor detector designed. 
%
...
%the reconstructed invariant mass of the charged particle pair, e.g  $M_{l^\pm
%\pi^\mp}^2=m_l^2+m_{p^\pm}^2+ 2(E_l E_\pi -
%|P_l||P_\pi|\cos\theta_\text{sep})$ for $\ster\rightarrow \pi^+ l^-$, which
%sum to that of the the parent sterile (within detector resolution), where as
%the background is expected form a broad spectra spanning the energies of
%incoming neutrino. Additionaly, two body decays allow for reconstruction of
%the parent sterile angle with respect to the beamline which is assumed to be
%close to on-axis, unlike the more isotropic backgrounds. We found $\approx
%95$\% of reconstructed sterile from two bodys decays were below $4^\circ$
%relative to the beamline and we cut all events greater than this. 
%
