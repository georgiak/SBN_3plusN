...
\abstract{
%
We study the sensitivity of the Short-Baseline Neutrino (SBN) program at
Fermilab to $\mathcal{O}$(MeV) sterile neutrino decay-in-flight in both a
minimal extension and more generic beyond the Standard Model scenarios. Such
states would be naturally produced in decays of the secondary meson beam,
propagate to the SBN detectors and subsequently decay into visible particles
allowing for a complementary new physics search to standard eV-scale
oscillations, without the need for any additional beam or detector modifications. We demonstrate that the programme can, in many
cases, extend existing bounds on $\ster \rightarrow \nu_\alpha l_1^+ l_2^-$ and
$\ster \rightarrow l_1^\pm \pi^\mp$ ($l_1,l_2 \in e,\mu$) whilst, due to the strong particle identification
...
\nu_\alpha \pi^0$, which may be significantly enhanced in a non-minimal model.
Additionally we highlight that if the light detection system employed by SBND
and ICARUS can achieve $\approx$nano-second timing resolution, it in principle allows
for the observation of events populating outside the neutrino beam-buckets, due to the sub-luminal propagation of the heavy sterile. 
When combined with spectral energy information, this allows SBN to act in a near backgroundless environment, whilst also providing a
distinctive smoking-gun signature for this class of models.
%
}
...
literature to explain this observation in conjunction with the lightness of neutrino
...
for a new physics scenario distinct to that of oscillatory sterile neutrinos.
...
compatible with the primary goals of the SBN, and requires no modifications to
...
provides an ideal scenario to study this ``decay-in-flight'' of sterile neutrinos.
...
resolution of scintillation light is achieved, the timing structure of the markedly
...
Bernardi:1987ek}, a beam dump experiment which ran at CERN from in 1984. 
%
...
produced by the 19.2 GeV CERN Proton Synchrotron beam used at PS-191. As we discuss
in more detail in \refsec{sec:decays}, the probability that the sterile neutrino
decays inversely scales with momentum, $1/|P_\ster|$, and we would therefore expect any BNB detectors to
see more events than PS-191 for equivalent neutrino exposures.
...
signal. We then study how the event timing information could be used to prove any observed excess is indeed originating in such a decay-in-flight scenario and help
...
competitive for higher masses.\marcom{PB}{We should check this.  Aparici et al.
don't say what happens to LHC bounds in this region.} This could also be
related to the enhanced $\ster \to \nu\gamma$ decay rate introduced in
...
%are forbidden purely from the inferred bound from an experimentally
%constrained channel.  % However, it is worth pointing out that an enlarged
%decay rate would not invalidate any bound placed experimentally on a given
%channel. In fact, reporting the results of the non-observation of certain
%decay products in terms of a mixing matrix element in the minimal extension is
%mostly harmless. We can always see this as an effective quantity. 
%
...
%non-observation of a signal in a given decay channel can still be used to
%bound the mixing matrix element, and therefore the minimally-correlated bounds
...
visible particles. The main consequence of this is that there is no \emph{a
priori} relationship between the magnitude of individual decays --- a single
channel may be enhanced beyond its value from \refsec{sec:minimal} --- and
bounds inferred from the non-observation of a given channel, may not hold in 
a non-minimal model when applied to another channel.
...
Big-Bang Nucleosynthesis (BBN) by both speeding up the expansion of the universe
...
baseline dependence, e.g $(470/540)^2 \approx 1.3$ at \muboone. This is
...
%
This means the effective spectrum of \emph{decaying} particles is biased
...
weight factor for that energy. As such the full spectral shape of the signal is
included in the rate analysis. As a consistency check of our methodology, we
...
kinematic cuts there is the potential for even greater background suppression by
utilising the time of arrival of observed events. Although the drift time of
electrons in LAr can be as large as $\mathcal{O}$(100)$\mu$s, the ionization
and excitation of Argon from the passing of a charged particle also produces
128nm scintillation light of which there is a nano-second scale contribution
from the excited state $Ar^*_2$ decaying \cite{Acciarri:2015hha}. LAr is
transparent to this light so if the light detection system (LDS) employed by
the SBN detectors has a similar nanosecond resolution, this can allow for
...
to limit constant rate backgrounds such as cosmogenic events. The LDS of both
SBND and ICARUS, however, are expected to be able to achieve significantly
better timing resolution than this, $\approx \mathcal{O}$(1-2)~ns depending on
the exact technology used, which potentially allows for the use of both bucket
...
form the 19.2$\mu$s spill with a frequency of 3Hz \cite{Antonello:2015lea}.  If
this nano-second resolution is indeed achieved, it allows for events in
individual buckets to be identified. Such a nano-second resolution was achieved
previously by the PMT's utilised in MiniBooNE \cite{Antonello:2015lea}, with
potential for improvement in the next generation SBN detectors. \muboone\ is
omitted from such a timing related analysis as the achievable timing resolution
is lower at around $10$~ns \cite{Katori:2013wqa}.
%
...
begins to become relevant when the sterile neutrinos are of MeV-scale masses
and above. As the flux of decaying sterile neutrinos is inversely proportional
to its momentum after convoluting with their decay probability, many of these
low energy steriles are traveling at sufficiently slower speeds than their
light counterparts to be distinguishable. Shown on the right of\reffig{fig:timing} is
the fraction of events that are expected to fall outside the  bucket window in
both SBND and ICARUS, with detailed timing spectra of a $25$ and $350$ MeV
sterile shown on the left \reffig{fig:timing}, we use polar plots to highlight the cyclic nature of the buckets as well as to point out that the absolute timing is not known as an event could originate from any previous bucket, only the relative timing shift, $\Delta_t$, modulo 21 is known. For
the purposes of this study we define the beam-correlated window to be a $6$~ns
period, $2$~ns each side of the $2$~ns beam bucket.

As can be seen a significant proportion of sterile events are distributed
throughout the inter-bucket area. Events which fall into the beam-bucket timing
window have to be analysed on top of all known beam-related backgrounds, where
as the increasingly larger fraction which populate the inter-bucket window have
extremely reduced beam-correlated backgrounds. Cosmogenic backgrounds are not
expected to play a major role, as the combination of fiducial veto, trajectory
and particle composition can be well understood from in-situ inter-spill and
beam-off measurements. In the main $\nu_e$ analysis, the single electron signal is easily mimicked by cosmic $\gamma$ backgrounds but despite this rates of only  146 (88,164) events in SBND (\muboone, ICARUS) after fiducial and $dE/dx$ cuts \cite{Antonello:2015lea}, are expected. As the decay in flight signatures are much more complicated, e.g $\pi \mu$ originating at a single point coming directly along the beamline, we believe cosmogenic backgrounds will not be a problem. A detailed analysis of the potential non-beam related backgrounds  would be necessary but is beyond the scope of this work.


The implication of the timing effects is that our discussion of backgrounds is
twofold: for low mass steriles, or any mass at \muboone\ due to its worse timing
...
\resizebox{\columnwidth}{!}{\input{figures/spectrum.tex}}\\
...
top left panel of \reffig{fig:spectrum_ee}.\marcom{PB}{Cut one of these
figures?} For the lowest masses that we consider, almost all events have
...
our cuts. In the bottom panel of \reffig{fig:spectrum_ee} we show the same
distributions for electron-pion production, noting the same spectral features
of the electron-positron channel.  In the inset we also show the differences
that a good timing resolution can give, with the in-bucket and out of bucket
spectra showing significant differences.  Through optimisiation of this part of
...
numbers of sterile events expected, the $\mu$BooNE
experiment is significantly more advanced than its two SBN counterparts. At the
time of writing, \muboone\ has already collected $\approx 50$\% ($\approx
3.5\times 10^{20}$ POT) of its planned POT and has presented its first results
on $\nu_\mu$ CC inclusive and $\nu_\mu$ CC$\pi^0$ \cite{mubooneneutrino}. As
such \muboone\ is in a unique position in that it has the
potential to observe a possible excess long in advance of SBND or ICARUS 
and thus to inform a possible search using the full SBN programme.
...
%$|U_{\mu 4}|^2 \approx 10^{-7.2}$) would mean that SBND will expect to see $\approx 2000$
%events for the same parameter space, \muboone\ would see such a signal several
%years before SBND.
Non-observation of any excess at $\mu$BooNE would not negatively effect the program for a subsequent search at SBND or ICARUS as a large amount of their probable parameter space is not available to $\mu$BooNE.
...
As can be seen the increased event rate of sterile decays at the SBN program in
comparison to PS-191 has the potential for the improvement on the bounds on
$|U_{e4}|^2$ and $|U_{\mu 4}|^2$ in all channels studied for wide regions of
parameter space even when taking into account the increased rate of backgrounds
expected. The strongest bounds come from semi-leptonic $\ster \rightarrow l^\pm
\pi^\mp$ searches, with mixing greater than $|U_{e4}|^2 \leq 10^{-9}$ excluded
...
can indeed be keep under control by sufficient timing resolution. 

Additionally, the previously unbounded $\ster \rightarrow \nu_\alpha \pi^0$ and
$\ster \rightarrow \nu_\alpha \gamma$ can be probed across the entire parameter
space providing new constraints on exotic sterile signatures, although the
potential backgrounds are larger in these photonic channels leading to a wider
spread in achievable limits. For steriles  $m_\pi^0 \leq m_\ster \leq m_\pi^\pm
+m_\mu$ the  $\ster \rightarrow \nu_\mu \pi^0$ channel can extend the limits
beyond that of the traditional $e^+e^-$ searches, even in the minimal model,
although in the case of electron mixing the $\pi^0$ bounds are always below
that of the semi-leptonic $\ster \rightarrow e^\pm \pi^\mp$.  

\begin{figure}[t]
\center
\includegraphics[width=1.0\textwidth]{figures/band_sbn_new.pdf}

\caption{\label{fig:band_sbn}The predicted  90\% C.L  upper limit contours for the
...
indeed from a heavy particle propagating the baseline before decaying. Timing is
...
that mimic the energy spectrum. As we expect all beam-related backgrounds to be in line with the booster proton buckets, observation of events with timing 
...
with beam-related models by generating many Monte-Carlo
experiments simulating such a decaying sterile and analysing the generated data under
...
of mass m.  As the timing is solely a function of initial sterile energy and mass, these results hold for all channels studied, some results from this analysis for the $\ster \rightarrow e^\pm \pi^\mp$ channel are shown in \reffig{fig:hockey} . On the left we assume ICARUS observes an excess of 100 events due to a 300 MeV sterile. To guarantee that we obtain $3\sigma$ significant rejection of the beam-window hypothesis 95\% of the time ICARUS would need a timing resolution of $\leq$ 3.5 ns. The greater the observed excess the proportionally less stringent timing resolution is required, as can be seen in the right-hand plot of \reffig{fig:hockey}.
...
decay in flight. We use the same test statistic as used in the temporal analysis
...
%Left: The spectral differences in total visible
...
in LArTPCs. We argue that a sufficiently good timing resolution, $ \approx 3$ns, can play a key role in the reduction of
