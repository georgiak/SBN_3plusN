...
allowing for a complementary new physics search to standard eV-scale oscillations, without the need for any beam or detector modifications. We
provide estimates for the bounds that the full SBN programme can place on the
parameter spaces of these models and demonstrate that the programme can in many
...
\nu_\alpha \pi^0$, which may be significantly enhanced in a non-minimal model. Additionally
we show that the if the light detection system employed by SBND and ICARUS can achieve
nano-second timing resolution, it in principle allows for observable structure
in the temporal distribution of events relative to the neutrino beam-buckets. When
combined with spectral energy information, this allows SBN to act in a
near backgroundless environment, whilst also providing a distinctive signature
...
In a conventional neutrino beam, most neutrinos are derived from meson decay, and we assume that the sterile neutrinos are
produced in similar channels with suppression from a mixing matrix element
$U_{\alpha 4}$. We restrict ourselves to
sterile masses below the Kaon mass, $m_{\ster}\lesssim m_K$. 
%
Larger sterile masses can be probed by working at higher energies, where the
neutral fermions come from decays of charmed mesons such as $D^\pm$. This strategy 
will be used by the upcoming SHiPS experiment \cite{Alekhin:2015byh,
Anelli:2015pba} but will not be considered futher in the paper as D mesons are produced in
extremely small numbers due to the relatively low energy protons of the BNB beam \cite{AguilarArevalo:2008yp}. \\

Although novel dynamics may lead to enhanced production rates of sterile
neutrinos by alternative unconventional means, we neglect this possibility and assume that
the sterile component of the BNB flux arises soley from such meson (or secondary $\nu_\mu$) decays only. This process
...
the fluxes for the active neutrinos. 

The final sterile fluxes can be estimated by taking known active neutrino fluxes and scaling them by the appropriate mixing $U_{\alpha 4}$, with an
additional kinematic factor to take into account the helicity un-suppression of
channels if the flavour pattern allows, such as $\pi^+ \rightarrow e^+\ster$ for massive $\ster \gg m_e$ if $U_{e4}$ is non-zero. As the meson decay comes with an associated charged lepton, mixing with the $\tau$-flavour cannot lead to production at the BNB, and so production is possible with $U_{e4}$, $U_{\mu 4}$ or a combination of the two.
...
for the significantly smaller intrinsic flux of $\nu_e$  inherent in the BNB, $\approx 0.52$\% \cite{AguilarArevalo:2008yp}. As
...
fermions produced by pion and kaon decays, $10~\text{MeV}\lesssim m_{\ster} \lesssim m_K$. The precise 
decay rates and branching ratios for these channels are model dependent. 
...
Although the electron-positron channel dominates the decays at $m_\ster \leq m_\pi^0$,
...
\newtext{MARK}{Haven't changed this section yet, just seperated it out}
...
\newtext{MARK}{another bound for btoh minimal and non-minimal}
...
final state particles only. The kinematics of signal decay-in-flight and backgrounds from scattering events, however,  have strikingly different behaviour leading to strong supression capalities.

All two body decays containing visible daughters have a powerful discriminator in
...
As a representative example we show here the backgrounds associated with the decay $\ster \rightarrow \mu^\pm \pi^\mp$. The dominant backgrounds to this channel will be genuine chargedcurrent $\pi \mu$ production associated with the $\nu_\mu$ neutrino beam of which there are many expected at the SBN detectors. In essence the signal of a traditional neutrino scattering experiment become the backgrounds for a decay-in-flight analysis. We show in \reffig{fig:mu_pi_cutflow} the power of such a kinematic analysis as each cut is implemented, reducing the inclusive $\mu \pi$ eventrate at $\mu$BooNE from $\approx 100,000$ to $\approx 30$ with a signal efficiency of 56\%. 
