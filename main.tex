...
\cite{XXX} to radiative mass generation \cite{XXX} or even more involved
constructions \cite{XXX}. It will ultimately be the role of phenomenology to
find ways to distinguish between candidate models, and see what can be learnt
from the completion of the neutrino sector about the structure of BSM physics.
%
...
%
Ignoring flavour effects for now, this means there is essentially a single
parameter which dictates the magnitude of all the decay rates. This is a great
asset when trying to constrain the  minimal model, for example it allows us to
reinterprete bounds on any one decay rate onto the observation of other
channels, such as $\nu_N \rightarrow \nu_\alpha e^+ e^-$ non-observation being
...
example a vertex allowing $N\to h \nu$ or $N\to \nu Z$ at a rate governed by
the scale of new physics suppressing these operators.

As a concrete example, a model with a leptophilic $Z^\prime$ could enhance the
rate $N\to \nu e^+\mu^-$ while not enhancing $N\to e^\pm \pi^\mp$.
\marcom{PB}{Can I back this up with a reference?}

$N\to \nu \gamma$: However, in a non-minimal model this decay rate could be enhanced. For
