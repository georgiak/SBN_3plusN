...
Fermilab to $\mathcal{O}$(MeV) sterile neutrino decay-in-flight. These
particles would be naturally produced in decays of the secondary meson beam,
propagate to the SBN detectors and subsequently decay into visible particles.
The search for such decays is a complementary new physics analysis to the
standard eV-scale oscillations which requires no further beam or detector
modifications. 
%
We discuss the decays which might be visible at SBN in a minimal and
non-minimal extension of the Standard Model, and perform a simulation to
compute the parameter space constraints which could be placed in the absence of
a signal.
%
We demonstrate that the programme can extend existing bounds on well
...
\pi^0$. 
%
Furthermore, we consider the phenomenological impact of improved timing
information at the three detectors. If the light detection system employed by
SBND and ICARUS can achieve $\approx$nano-second timing resolution, the effect
of finite sterile neutrino mass may be observable as events are expected to
arrive outside the neutrino beam-buckets. When combined with spectral energy
information, this allows SBN to significantly reduce its beam-correlated
background for a search for new physics, while through the comparision of event
timing distributions at near and far detectors, can also provide a smoking-gun
...
to protect them) are unstable, and their
...
which there is significant production from meson decay. In this mass range, the
...
which ran at CERN from in 1984. 
%
...
performance. We return to this issue in \refsec{sec:backgrounds}.
...
production associated with the $\nu_\mu$ neutrino beam. In essence the signal
of a traditional neutrino scattering experiment become the backgrounds for a
decay-in-flight analysis.  This channel, and indeed $e^\pm \pi^\mp$, has a
powerful discriminator in the reconstructed invariant mass of the charged
particle pair, e.g  $M_{l^\pm \pi^\mp}^2=m_l^2+m_{p^\pm}^2+ 2(E_l E_\pi -
|P_l||P_\pi|\cos\theta_\text{sep})$ for $\ster\rightarrow \pi^\pm l^\mp$, which
sum to that of the the parent sterile (within detector resolution), where as
the background is expected form a broad spectrum spanning the energies of the
incoming neutrinos. Additionally such two body decays allow for reconstruction
of the parent sterile angle with respect to the beamline which, as the sterile
impinged the detector, is very close to on-axis unlike the more isotropic
backgrounds. We found that approximatly $95$\% of the reconstructed sterile 
angles from such two body decays were inside a $4^\circ$ cone centered on the beamline. 
...
eventrate at $\mu$BooNE (SBND) from $\approx 10^5, (1.5\times10^6)$ to $\approx
...
electrons in LAr can be as large as $100$~$\mu$s, the ionization
...
from the excited state $\text{Ar}^*_2$ decaying \cite{Acciarri:2015hha}. LAr is
...
better timing resolution than this, around $1$--$2$~ns depending on
...
to its momentum after convolving with their decay probability, many of these
