...
\item \sout{PS191 replicate the bounds.} \newtext{MARK}{Consistently a factor
of $\approx 100$ off. What Ive checked. Fluxes seem good. We are using same
$\Gamma_{\nu ee}$, up to I integral. POT is good. L, and $\lambda$ as well as
other dimensions seem good. Hmm. Effect is really only in $\vert U_{\mu
4}\vert^2$ and not electron, checking meson-BR. Not meson-Br. FIXED! although
still $\approx 50$ off. Lets say conservative.} We are still missing that
factor of 50. Total decay rate is irrelavent, only channel is important. is
channel correct?

\item What can we do with enhanced decay(/production?) rates? Significant
baseline dependence? Observable differences with SM signal (or does everything
scale boringly)? How can we show this? I suspect that we cannot by introducing
new physics argue that signals could exist in excluded parameter space. \newtext{PB}{Let me know if you want to look at this today: 16 Feb.}

\item Similar analysis for DUNE ND? Generic detector? \newtext{PB}{Let me know if you want to look at this today: 16 Feb.}

\item If we go the GENIE route: make inflight's output LHE format.
