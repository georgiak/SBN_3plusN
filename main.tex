...
oscillations, without the need for any additional beam or detector modifications. We
provide estimates for the bounds that the full SBN programme can place on the
parameter spaces of such models and demonstrate that the programme can, in many
...
\cite{Akhmedov:2009rb}\marcom{PB}{Is this an accurate description? Mark: I guess its technically $Delta E$ but thats almost equivalent here. I think its ok}.  
%
...
any events which are below the sterile mass threshold are removed and the remaining flux is scaled accordingly.
\marcom{PB}{This sounds a bit dodgy. How do we chose where to move
them to? First bin? Shared over all bins? Can we clarify? Just re-normalising the sterile flux to have same number of events as neutrino after chopping. It is dodgey in that we know there would be spectral shifts.}
...
by \marcom{PB}{Can we add a line about how? MARK: I don't quite understand, how they decay by charge current only?}
%
...
simplified form depending on the size of $\Gamma_\text{T}\lambda/\gamma\beta$:\marcom{PB}{Only the upper one applies to us, right? Is this unnecessary? The lower one applies in the extended case so  err on keeping.}
%
\begin{align} 
%
...
above.  Of crucial importance to this is accurate knowledge of active neutrino
fluxes at all three SBN detectors. These are calculated from published
MiniBooNE fluxes \cite{AguilarArevalo:2008yp}, after scaling by appropriate
$1/r^2$ baseline dependence, e.g $(470/540)^2 \approx 1.3$ at \muboone. This is
...
probability for any given sterile to subsequently decay inside a detector scales as
$1/|P_{\ster}|$\marcom{PB}{What did you mean by this, Mark? E? No its 1/momentum scaling, so correct here.} the fluxes of
active neutrinos at low energy are of particular importance, in stark contrast
to standard neutrino interaction cross sections, which scale as $E_\nu$. 
%
...
analysis cuts or detector performance effects. \marcom{PB}{What do you think
about a plot of our weight functions from the cuts? The cuts are very background dependant so if it were in it would make more sense in appendix or after background have been discussed. I think its unnecessary here}
%
...
is lower, $\approx 10$ns \cite{Katori:2013wqa}.\marcom{PB}{Can we comment on why? I believe it comes from fewer PMT's however I cannot find an accurate source, Andzrej was the first person to highlight it so I will ask him. }
...
extremely reduced beam-correlated backgrounds. Cosmegenic backgrounds are not expected to play a major role, as the combination of fiducial veto, trajectory and particle composition can be well understood from in-situ inter-spill and beam-off measurements \cite{Antonello:2015lea}.   \marcom{PB}{We need to comment on cosmogenics.} 
...
numbers of sterile events expected, and hence sensitivity, the $\mu$BooNE
...
such \muboone\ is in a unique position in the SBN program in that it has the
potential to observe  a possible excess long in advance of its SBN counterparts
and thus to inform a possible search using the full SBN detectors. An example
of this is if \muboone\ observed an excess of 100 events consistent with a 300
MeV sterile decaying $\nu_\ster \rightarrow \nu_\mu \pi^0$ (corresponding to
$|U_{\mu 4}|^2 \approx 10^{-7.2}$), SBND would expect to see $\approx 2000$
events for the same parameter space. \muboone\ would see such a signal several
years before SBND, however, and as such any anomalous observations in \muboone\
possibly consistent with a decay-in-flight would strongly encourage SBND and
ICARUS to ensure they achieve the target $~1$~ns resolution needed to prove the
anomaly is indeed from the decay of a heavy propagating sterile. Non-observation of any excess at $\mu$BooNE would not negatively effect the program for a subsequent search at SBND or ICARUS as a large amount of their probable parameter space is not available to $\mu$BooNE.  \marcom{PB}{We
should mention consequences of non-observation at microboone.}
...
$\ster \rightarrow \nu_\alpha \gamma$ can be probed across the entire parameter space providing new constraints on exotic sterile signatures, although the potential backgrounds are larger in these photonic channels leading to a wider spread in achievable limits. For steriles  $m_\pi^0 \geq m_\ster \leq m_\pi^\pm +m_\mu$ the  $\ster \rightarrow \nu_\mu \pi^0$ channel can extend the limits beyond that of the traditional $e^+e^-$ searches, even in the minimal model, although in the case of electron mixing the $\pi^0$ bounds are always below that of the semi-leptonic $\ster \rightarrow e^\pm \pi^\mp$.  

\marcom{PB}{This needs expanding.}
