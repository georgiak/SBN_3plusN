...
int ntupleProcess(std::string xml){
  ...
  // Create our tree
  TTree * t_dis = new TTree("numudis","numudis");
  ...
  for(int i = 0; i < v_chi2.size(); i++){
    ...
    float mysin22th = pow(sin(2*v_theta24[i]),2);
  ...
  TTree * t_dis_90 = new TTree("numudis_90_excl","numudis_90_excl");
