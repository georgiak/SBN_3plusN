...
double MicroBooNE_dis::Chi2(Oscillator osc, neutrinoModel model,bool debug, double* data_external){
  ...
  if(shapeonly){
    ...
    for(int iB = 0; iB < nBins; iB++){
  		for(int jB = 0; jB < nBins; jB++){
        if(statsonly)
  			   covMatrix[iB][jB] = 0;//(Full_fractCovMatrix[iB][jB] - fnorm) * Prediction[iB] * Prediction[jB]; // remove normalization component of cov matrix
        else
        ...
        if(iB == jB){
          if(MCStatSquared[iB]==0)
            std::cout << "MATRIX WILL BE SINGULAR. PLEEZ FIX" << std::endl;
          else
    ...
    for(int iB = 0; iB < nBins; iB++){
        ...
        if(statsonly)
           ...
           covMatrix[iB][jB] = Full_fractCovMatrix[iB][jB] * Prediction[iB] * Prediction[jB];
			  ...
			  if(iB == jB){
          if(MCStatSquared[iB]==0){
            std::cout << "GOT EMPTY BINS. FIX THAT. PUTTING IN SMALL TERM FOR NOW" << std::endl;
            covMatrix[iB][jB] = 0.0001;
          }
          else
				    covMatrix[iB][jB] += MCStatSquared[iB] + Prediction[iB]; // Add statistical error of signal prediction
			  }
