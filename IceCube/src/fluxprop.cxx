...
int fluxprop(int part){
  ...
  std::ofstream file("libflux/flux_"+partsrc+"_nominal_avg.txt");
  ...
  // We're gonna average 4 evaluations per bin.
  int nZBins = 21;
  int nEBins = 200;
  int nAvg = 4;
  ...
  // Now put it all together and write that shit to file
  ...
  for(int cz = 0; cz < nZBins; cz++){
    for(int re = 0; re < nEBins; re++){
      ...
      file << " " << centerCZ << " " << centerE;
      file << " " << flux_nu/(nAvg*nAvg) << " " << flux_nubar/(nAvg*nAvg);
      file << std::endl;
    }
  }
  file.close();
