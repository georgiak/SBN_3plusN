...
for chi in `seq 0 0.1 2.5 `
do
		TOTALD=0 #$(wc -l < fractiondata/NUBAR_MODE/pot3p3/3p3_pot_0.00_-2.00_dis.bar.dat)
		...
		TOTALA=$(wc -l < fractiondata/NUBAR_MODE/pot3p3/3p3_pot_0.00_-1.00_app.bar.dat)
 		echo "On chi "$chi
		ANSD=0 #$(awk -v mychi="$chi" -v count=0 '{ if ($17 >= 10^mychi) count++}  END {print count}' fractiondata/NUBAR_MODE/pot3p3/3p3_pot_0.00_-2.00_dis.bar.dat)
